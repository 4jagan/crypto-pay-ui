...
export default async function VolumeSpike() {
    const data = await getDashboardData();
    
    if (!data) {
        return <div className="p-4 text-gray-500">Loading...</div>;
    }
    const { riskScore, riskLevel, description } = data.anomalyDetection.volumeSpike;
    ...
    return (
        <div className="flex flex-col justify-between h-36 w-72 shadow-xs rounded-lg p-4 bg-gray-100 text-gray-800 border border-gray-300">
            <div className="flex justify-between items-center">
                <h3 className="font-bold">Volume Spike</h3>
